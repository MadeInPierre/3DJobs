...
function reset_chart_messages() {
    while(messages_charts.lastChild) {
        messages_charts.removeChild(messages_charts.lastChild);
    }
}
...
form_charts.addEventListener('submit', function(e) {
    ...
    if(select_chart.value == 'users_statistics_prices') {
                ...
                if(req.status == 200) { // If OK, server returns a JSON object containing the statistics
                    ...
                    for(var i = 0; i < response.length; i++) {
                        if(spoolid_to_i[response[i].spool_id] === undefined) { // Spool has not been added to dataset yet
                            ...
                            var bg_color_weight = 'rgba(173, 212, 167, 0.7)';
                            var border_color = tinycolor(bg_color).darken(20).toString();
    ...
    if(select_chart.value == 'users_statistics_weights') {
        var m_from = users_statistics.getElementsByTagName('select')[0].value;
        var d_from = users_statistics.getElementsByTagName('select')[1].value;
        var y_from = users_statistics.getElementsByTagName('select')[2].value;
            ...
            if(req.readyState == 4) {
                if(req.status == 200) { // If OK, server returns a JSON object containing the statistics
                    ...
                    for(var i = 0; i < response.length; i++) {
                        ...
                        // And now, we add user data to spool dataset
                        chart_configuration.data.datasets[spoolid_to_i[response[i].spool_id]].data[userid_to_i[response[i].user_id]] = response[i].total_weight; // Remember : 2nd spool index is for weight
                    }

                    var from_date = new Date(y_from, m_from, d_from).toDateString();
                    var to_date = new Date(y_to, m_to, d_to).toDateString();
                    chart_configuration.options.title.text = 'Users statistics (weights) from ' + from_date + ' to ' + to_date;
